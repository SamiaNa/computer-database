...
public enum ComputerMapper {
    ...
    public List<Computer> createComputerListFromResultSet(ResultSet res) throws SQLException {
        ...
        while (res.next()) {
            computers.add(new Computer(res.getLong(1), res.getString(2), toLocalDateOrNull(res.getDate(3)),
                    toLocalDateOrNull(res.getDate(4)), new Company(res.getLong(5), res.getString(6))));
        }
        return computers;
    }
    ...
    public Optional<Computer> createComputerFromResultSet(ResultSet res, long id) throws SQLException {
        if (res.next()) {
            Computer c = new Computer(res.getLong(1), res.getString(2), toLocalDateOrNull(res.getDate(3)),
